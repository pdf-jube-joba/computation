...
fn shrink_bar_right_till_1_or_bar() -> TuringMachineBuilder {
    ...
    builder.init_state(state("start")).accepted_state(vec![state("end1"), state("end-")])
        .code_new(
            vec![
                "-, start,  , next, L",
                " ,  next,  , next, L",
...
pub fn projection(n: usize, i: usize) -> TuringMachineBuilder {
    let graph = GraphOfBuilder {
        ...
        assign_vertex_to_builder: vec![
            ...
            vec![shrink_bar_right_till_1_or_bar(), move_1_left_in_bar(), shrink_bar_right_till_1_or_bar()],
        ].into_iter().flatten().collect(),
        assign_edge_to_state: series_edge_end_only(n + 4),
        acceptable: accept_end_only(n + 4)

...
print("[INFO] files list:", target_dir)
...
for md_name in md_names:
    ...
    for name in list_component:
        head_load = head_load + load_head_html(name)
        body_load = body_load + load_body_html(name)
    ...
    body_load = '<script type="module">import init from \'/' + name + '.js\';init(\'/' + name + '_bg.wasm\');</script>'
    head_html = "<head>" + match_obj.group(2) + head_load + "</head>"
    body_html = "<body>" + match_obj.group(4) + body_load + "</body>"

...
### ちゃんとした定義１？
- 値：\(V \subset M\) := \(\N + \T{\#true} + \T{\#false} + \T{\#unit} + \text{Fun}(x, M) + \text{Rec}(f, x, M)\)
...
(\T{rec} f x \T{=>} M) V &\to M[f := \T{rec} f x \T{=>} M][x:=V] \\
V M &\to \bot && V \neq (\T{fun} x M) \\
...
> [!Note]
> rec は値に入れずに \(\T{fun} x => (M[f := \T{rec} f x \T{=>} M])\) に reduction されるとしてもいい。

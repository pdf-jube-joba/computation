...
| 右側を \(n\) の値を下げながらコピーし、 \(n = 0\) になったら \(p\) のみをコピーする。 <br> 返ってくる必要はない。| `... ? - x l x - U(n) F(p) x - U(n-1) F(p) x ... x - l F(p) x \| x \| F(p) ...` |
| \(M_z\) を呼ぶ。| `... ? - x - U(n) F(p) x - U(n-1) F(p) x ... x - l F(p) \| x \| - U(f_z(p)) x ...` |
| （☆）現在の状況 | `... ? x l x - U(m + 1) F(p) x - U(m) F(p) \| x \| - U(j) x` ただし、 \(j = P[f_z, f_s](m - 1, p)\) |
| **もし** 左側が `... ? x l` なら、 `x l` を除去し間を詰めて終了する。| `? x l \| x \| - U(j) x` から `? \| x \| - U(j) x` |

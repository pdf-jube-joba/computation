...
impl Machine for RecTmIrMachine {
    ...
    fn step(self, _rinput: Self::RInput) -> Result<StepResult<Self>, String> {
        let mut next = self;
        let (stmt, block_idx) = match next.next_stmt()? {
            Some(stmt) => stmt,
        ...
        match stmt {
            Stmt::Lt => {
                next.tape.move_to(&Direction::Left);
            }
            Stmt::Rt => {
                next.tape.move_to(&Direction::Right);
            }
            Stmt::Assign { dst, src } => {
                let value = next.eval_rvalue(&src);
                next.assign_lvalue(&dst, value);
            }
            Stmt::Break { cond } => {
                if next.eval_condition(&cond) {
                    let next_block = block_idx + 1;
                    next.frame.pc = (next_block, 0);
                }
            }
            Stmt::Continue { cond } => {
                if next.eval_condition(&cond) {
                    next.frame.pc = (block_idx, 0);
                }
            }
            Stmt::Jump { label, cond } => {
                if next.eval_condition(&cond) {
            ...
            Stmt::Call { func } => {
                let callee = func;
                let caller = Frame {
                    function: next.frame.function.clone(),
                    env: next.frame.env.clone(),
                    blocks: next.frame.blocks.clone(),
                    pc: next.frame.pc,
                };
                next.stack.push(caller);
                next.frame = build_frame_for_function(callee.as_ref());
            }
        }

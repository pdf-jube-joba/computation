...
where
    T: Clone + From<Ext<T>>,
{
    pub fn decomp<F>(t: Ext<T>, is_value: F) -> Option<(ExtFrame<T>, T)>
    where
        F: Fn(T) -> Option<ExtValue<T>> + Clone,
    {
        match t {
            ...
            Ext::App { e1, e2 } => match is_value(e1.clone()) {
    ...
    T: LambdaExt + From<Ext<T>> + Clone,
{
    ...
    fn subst_t(self, v: Var, t: T) -> T {
        match self {
            ...
            Ext::App { e1, e2 } => Ext::n_a(e1.subst(v.clone(), t.clone()), e2.subst(v, t)).into(),

...
## 仮想レジスタと control flow graph
用語を整理する。
- 値：計算が生成・消費する対象、単なるデータ。
- アドレス値：値のうち、ある場所を指し示す参照と解釈されるもの。
    - reference をとる: \(\NT{place} \to \NT{addr}\)
    - dereference をとる: \(\NT{addr} \to \NT{place}\)
    - ただし、ここでは addr は \(\N\) によってメモリを指定している
        - これは計算の対象としている普通の値と同じ扱いになっている：つまり、値とアドレス値を同じとして考えていい。
- 場所・メモリセル：書き込み先として指定できる対象。
    - \(\text{ld}(p: \NT{place}): () \to \N\) ... 場所から値を取り出す
    - \(\text{st}(p: \NT{place}): \N \to ()\) ... 場所へ値を入れる
...
\NT{place}  &\defeq \T{\LCB} \sp \NT{addr} \sp \T{\RCB} \\
...
reference/dereference をとるのが見えにくいのはこういう理由：
\(\T{\LCB} \NT{addr} \T{\RCB}\) でしか place を使えないので、
dereference は \(\NT{addr}\) を取り出すだけ。
...
> [!Note]
> これ普通には意味を保ったままコンパイルができない。
> 例として、 \(n\) レジスタマシンを考える
> - セルは WORD 単位で自然数は入れられない
> - \(r_{i \in 0 \ldots n}: \N\) がレジスタでメモリは \(M := \N \to \N\)
> - stmt は \(r_i := r_j + r_k, r_i := r_j - r_k\) と \(\T{ifz} r_i \T{then} \N\) でレジスタのみから演算と分岐。
> - もしメモリアクセスが即値だけなら厳しいので、 \(\T{ld} [r_i] \T{->} r_j\) と \(\T{st} r_j \T{->} [r_j]\) を入れる。
> 
> \(n > m\) のときに \(n\) レジスタマシンから \(m\) レジスタマシンへ transpile できるか？
> 最終的なメモリの状態（ \(\text{code} \pfun (M \to M)\)）だけを問題にしたとしても、コンパイルがすごい難しい。
> 理由は、レジスタのメモリ退避をするためには、つかわれていないメモリを考えないといけないが、実行時にしかどのメモリにアクセスするかが予想できないから。
> 例えば、 \(n\) 側のメモリを再現するのを \(2 \N\) 部分だけでやることは可能かもしれないが、普通のやり方じゃない。
> 同様に、 WORD じゃなくて \(\N\) が入っているなら、１個のレジスタで \(2^{r_1} * 3^{r_2}\) みたいにやれば大きなレジスタを
> 毎回分解して扱うことができるが、これも普通のやり方じゃない。

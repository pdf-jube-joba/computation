...
ということは、 `sfree` はいらない。
これは、メモリ領域があるスタックに属しているとかの議論、 region 推論に似ている。
ある変数からたどることのできるヒープまで自動で開放する、とかができれば region 推論？

あとは全体的にサイズを指定してアクセスができるようになればいい。

## メモリを分割するのはやっていいのか？
線形なメモリがあるとして、それを任意の数に分割することはできる。
例： \(\N = 2 \N \cup 2 \N + 1\) を用いると、**普通の addressing とはぶつかるが**
`meven(i: index)` と `modd(i: index)` 経由以外ではアクセスしないと考えればいける。
だから、 Source 言語の側では単なる `*addr` をやめて上記のものしか入れないことにすればいい。

...
## refinement
とりあえず `.data` のラベルとレジスタを統合して、全部変数として考えてみる（変数の無限化）。
イメージとしては、すべて登場する変数にメモリを振っていて、レジスタとのやり取りを完全になくす。
...
\NT{stmt} &\defeq ( \\
    ...
    & ) \sp \T{;} \\
\NT{cond} &\defeq \sp \NT{atom} \sp \NT{rel} \sp \NT{atom} \\
\NT{cont} &\defeq \\
...
ただこれだと、ちょっと扱いにくい。
1. 「本当にメモリに入れておく必要があるもの」と「別にずっとレジスタにいてもいいもの」の区別がついてない。
  - 現在のものは、計算のために一時的に作られる値にもメモリを割り当てることになっている。
2. 初めに定義した変数が割り振られた領域にしかアクセスできない。
...
1. `<label>` でアクセスしている領域を無限に長い線形メモリとしてしまって、
  ...
  これはつまり、
  - アセンブリ言語の側ではそのまま"数値"でメモリにアクセスできていたものを、
  - さっきは変数名以外でアクセスできなくなったものを、
  - また数値でアクセスできるようにしただけ。
...
### コードへのジャンプの自由化
アセンブリからさらっと失われていた機能として、実行時にどこに飛ぶかを計算して与えることができない。
つまり、 `"goto" <label>` だけだとどの関数に飛ぶかを後から指定するのがちょっとめんどくさくなる。
...
なので、普通の最適化がしにくくなる。
とりあえず、ブロックの途中には飛べない（飛ばないものしかプログラムの実行として考えない）としておく。
...
> [!Note]
> 仮想レジスタという言葉を用いたのは、レジスタに対してのみ計算が行えるという特徴を引き継いだから。
> また、式もなく単なる binop しか使えないので、 `<value> <op> <value>` の形しか計算ができない。
> こういう部分を楽にやるなら、 `<assignable> := <value> <op> <value>` みたいにして、
> `<value>` に `<place>` からの load が入るとしてもいい。

...
あまり凝ってない。
field 名でアクセスできるようにするもの以外に、 tuple といって、番号でアクセスできるようにするものがある。
（ `x.0` みたいに。）
...
直積型との違いとして、実行時に何番目を使うかを計算していいことが多い。
```
fn a1() {
  a = [10, 2, 5, 3];
  x = a[f(3)]; // f(3) の呼び出し結果でアクセスする。
}

fn a2() {
  a = (10, 2, 5, 3);
  x = a.f(3); // こういうのはたいてい無理。
}
```

> [!Note]
> 理由は、直積型は異なるデータ型を抱えていることが多いので、
> offset の計算が複雑になったり、
> 型システムがうまく働かなくなったりするから？
...
```
これをメモリ上の連結した領域に置こうとすると、長さがデータ型に対して一定ではなくなりうる。
1. どうにか頑張って、長さが一定じゃないデータ型も扱えるようにする。
2. ポインタ経由で分散させる。 `rust` だと、後者を選ぶことを `Box` で明確に書かせる。

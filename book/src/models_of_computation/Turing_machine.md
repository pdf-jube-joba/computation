...
> **definition**
> - Turing machine \(M = (\Sigma, Q, q_{\text{init}}, Q_{\text{fin}}, \delta)\) を固定する。
...
計算が定義されたことから、２つのTuring machineが同じ計算を行っているかどうかを以下のように定義する。
> **definition**
...
## Turing machineができること：計算可能な自然数の関数について
自然数の符号化を固定して、Turing machineの計算できる自然数関数がどれぐらいなのかを見積もる。
ここでは具体的に、再帰関数と呼ばれるクラスを定義し、その関数がすべてTuring machineで計算可能であることを示す。

そのために、先に、Turing machineをより単純に構成できるように議論を行う。

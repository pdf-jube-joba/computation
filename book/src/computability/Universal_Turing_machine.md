...
チューリングマシンを自然数上のものとみなす。
: チューリングマシン \(M= (\Sigma, Q, q_{\text{init}}, Q_{\text{fin}}, \delta)\) に対して、 \(\Sigma\) も \(Q\) も有限集合なので、適当に全単射をとれば、ある自然数 \(n, m\) により、 \((\{0..n\}, \{0..m\}, q_{\text{init}}, Q_{\text{fin}}, \delta)\) と考えられる。
...
> [!Note]
> これは再帰関数と同じことができるという意味で、再帰関数がエミュレートできるというのとは違う？

上では記号を大量に用いたけど、各記号自体を \(4\) 字の \(\{b,1\}\) に符号化して、万能チューリングマシンの基本動作を「 \(4\) つ記号を読み取る」を中心に考えれば、

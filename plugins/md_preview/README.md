...
Set `[plugin.md_preview].macro_path` to copy a repository-local macros file into the built output as `macros.txt`.
...
For `workspace_fs` plugins, pass `{OUTPUT_DIRECTORY}` and configure any optional enhancers under `[plugin.md_preview]`.
